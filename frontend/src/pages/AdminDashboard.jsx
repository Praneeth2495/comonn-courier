      ...
      {loading ? <p className="lead">Loading…</p> : (
        <div className="table-wrap">
          <div className="t-row t-head orders-detailed">
            <div>Order ID</div><div>From Address</div><div>To Address</div><div>Qty</div><div>Order Status</div><div>Comment</div>
          </div>
          {orders.map((o) => (
            <div className="t-row orders-detailed" key={o.id}>
              ...
              <div>{o.items?.reduce((sum, it) => sum + it.quantity, 0) ?? 1}</div>
              <div>
                <select
