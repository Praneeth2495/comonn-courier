 ...
 * deleted afterward — see the cleanup query at the bottom of this file's
 * companion run (or scripts/load-test-cleanup.js).
 *
...
async function samplePostcodes(countryCode, n) {
  ...
  // Cycle through if we needed more than what's available (e.g. AE=7, SA=31, KW=102).
  const out = [];
...
async function main() {
  ...
  for (const cc of COUNTRIES) {
    ...
    const tasks = picks.map((pick, idx) => async () => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          errors.push({ idx, status: res.status, error: body.error || res.statusText });
        } else {
          success++;
        }
      } catch (err) {
        latencies.push(Date.now() - t0);
        errors.push({ idx, status: 'NETWORK', error: err.message });
      }
    });
    ...
    if (errors.length) {
      const distinctErrors = [...new Set(errors.map((e) => `${e.status}: ${e.error}`))];
      distinctErrors.slice(0, 5).forEach((e) => console.log(`   - ${e}`));
    }
    summary.push({ cc, success, total: ORDERS_PER_COUNTRY, countryMs, avg, min, max, errorCount: errors.length, errors: errors.slice(0, 5) });
  }
